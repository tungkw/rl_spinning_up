...
class myAgent(nn.Module):
    def __init__(self, env):
        super().__init__()
        self.env = env
        self.feature_dims = list(self.env.observation_space)
        self.action_dims = list(self.env.action_space.n)

        layers = [nn.Conv2d(4, 16, (8,8), 4), nn.ReLU(),
                  nn.Conv2d(16, 32, (8,8), 4), nn.ReLU(),
                  nn.Linear(32, 256), nn.ReLU(),
                  nn.Linear(256, self.action_dims[0])]
        self.action_value_model = nn.Sequential(*layers)

    def preprocess(self, o):
        gray_size = (110,84)
        crop_size = (84,84)
        gray = downsampling(cv.cvtColor(o, cv.COLOR_RGB2GRAY), gray_size)
        [h_d, w_d] = (np.subtract(gray_size, crop_size) / 2).astype(np.int)
